...
#22端口的封装
def get_port22():
    maxsingle = get_maxsingle("v3061_automation","0.0.0.0:")
...
def get_port5432():
	...
	# 将映射的端口2加入字典
	docker_dict['port_5432'] = port_5432
	return port_5432
...
def get_port443():
	...
	# 将映射的端口3加入字典
	docker_dict['port_443'] = port_443
	return port_443
	
#docker容器名的封装
def get_vesselName():
    maxsingle = get_maxsingle("v3061_automation","v3061_automation_")
    vessel_name = "v3061_automation_" + str(maxsingle + 1)
    docker_dict['vessel_name'] = vessel_name
    return vessel_name


def create_docker(docker_image):
     
	# 将镜像名加入字典
        docker_dict['image_name'] = docker_image
	# 将创建时间加入字典
	create_time = datetime.datetime.now()
	docker_dict['container_createtime'] = create_time
	
	#获取容器名赋值给变量
	vesselName = get_vesselName()

	#创建postgres容器
	dynamic_cmd='sudo docker run -d -p ' + get_port22() + ':22 -p ' + get_port443() + ':443 -p ' + get_port5432() + ':5432 --name ' + vesselName + ' -m 4G --memory-swap 5G --privileged --cap-add=SYS_ADMIN ' + docker_image + ' /usr/sbin/init /usr/sbin/sshd -D'
...
def container_process_status(vesselName):
	docker_cmd_execsh="docker exec -it " + vesselName + " /bin/bash -c 'supervisorctl status web:webcgi'"
	container_status = shell_cmd(docker_cmd_execsh)
	return container_process_status

def container_ip(vesselName):
	get_dockerIpCmd = "docker inspect --format '{{ .NetworkSettings.IPAddress }}' " + vesselName
	docker_postgresIp=shell_cmd(get_dockerIpCmd)
...
if __name__ == "__main__":
	...
	vesselName = create_docker("v3061/automation:s1")
	# 容器内业务初始化
	...
	docker_dict['container_group'] = container_group()
        # 获取容器内运行的服务状态
        docker_dict['container_process_status'] = container_process_status(vesselName)
        docker_dict['container_ip'] = container_ip(vesselName)	
	# 打印字典
